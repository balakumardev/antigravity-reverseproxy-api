/**
 * Shared constants (Node + edge runtimes)
 *
 * This file must stay free of Node-only builtins (`fs`, `path`, `os`, etc.)
 * so it can be reused by Cloudflare Workers.
 */

/**
 * Generate a runtime-specific User-Agent string.
 * @returns {string}
 */
function getPlatformUserAgent() {
    ...
    if (typeof process !== 'undefined' && process?.platform && process?.arch) {
        return `antigravity/${version} ${process.platform}/${process.arch}`;
    }

    // Workers / browsers
    return `antigravity/${version} worker/unknown`;
}
...
const oauthClientId = (typeof process !== 'undefined' && process?.env?.OAUTH_CLIENT_ID)
    ? process.env.OAUTH_CLIENT_ID
    : '1071006060591-tmhssin2h21lcre235vtolojh4g403ep.apps.googleusercontent.com';

const oauthClientSecret = (typeof process !== 'undefined' && process?.env?.OAUTH_CLIENT_SECRET)
    ? process.env.OAUTH_CLIENT_SECRET
    : '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';

const oauthCallbackPort = (typeof process !== 'undefined' && process?.env?.OAUTH_CALLBACK_PORT)
