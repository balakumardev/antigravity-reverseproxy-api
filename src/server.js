...
import { convertOpenAIToAnthropic } from './format/openai-request-converter.js';
import { convertAnthropicToOpenAI, convertStreamToOpenAI, formatOpenAIError } from './format/openai-response-converter.js';
...
app.post('/v1/chat/completions', async (req, res) => {
    try {
        ...
        if (accountManager.isAllRateLimited(model)) {
            logger.warn(`[Server] All accounts rate-limited for ${model}. Resetting state for optimistic retry.`);
            accountManager.resetAllRateLimits();
        }
        ...
        logger.info(`[API/OpenAI] Request for model: ${model}, stream: ${!!stream}`);
        ...
        if (stream) {
            ...
            try {
                ...
                const anthropicStream = sendMessageStream(anthropicRequest, accountManager, FALLBACK_ENABLED);
        ...
        // For auth errors, try to refresh token
        if (errorType === 'authentication_error') {
            logger.warn('[API/OpenAI] Token might be expired, attempting refresh...');
            try {
                accountManager.clearProjectCache();
                accountManager.clearTokenCache();
                await forceRefresh();
        ...
        logger.warn(`[API/OpenAI] Returning error response: ${statusCode} ${errorType} - ${errorMessage}`);

        // Check if headers have already been sent (for streaming that failed mid-way)
        if (res.headersSent) {
            logger.warn('[API/OpenAI] Headers already sent, writing error as SSE');
            res.write(`data: ${JSON.stringify({
            ...
            res.status(statusCode).json(formatOpenAIError({
                message: errorMessage,
                type: errorType
            }));
        }
